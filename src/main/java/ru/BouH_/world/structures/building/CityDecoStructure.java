...
public class CityDecoStructure extends AStructure {
    private Block footing;
    private Block footing2;

    public CityDecoStructure(StructureHolder structureHolder, int chance, BiomeGenBase... biomes) {
        super(structureHolder, chance, biomes);
        this.footing = Blocks.grass;
        this.footing2 = Blocks.dirt;
    }

    public CityDecoStructure(StructureHolder structureHolder, Block footing, Block footing2, int chance, BiomeGenBase... biomes) {
    ...
    private boolean checkBase(World world, int minX, int minY, int minZ, int maxX, int maxZ) {
        ...
        for (int i = minX; i <= maxX; i++) {
            for (int j = minZ; j <= maxZ; j++) {
                Block block = world.getBlock(i, minY, j);
                if (block == BlocksZp.road || block == BlocksZp.road2) {
                    return false;
                }
            }
        }
