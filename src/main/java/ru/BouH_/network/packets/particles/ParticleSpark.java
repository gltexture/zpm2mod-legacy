...
public final class ParticleSpark extends SimplePacket {
    public ParticleSpark() {
    }

    public ParticleSpark(int id) {
        buf().writeInt(id);
    }
    ...
    public void client(EntityPlayer player) {
        ...
        if (player.worldObj.getEntityByID(id) != null) {
            ...
            for (int is = 0; is < 16; is++) {
                ...
                Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleSpark(entity.worldObj, vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord + 0.1D, vec3.zCoord));
            }
        }
    }
}