...
public final class ParticleExplosionThermobaric extends SimplePacket {
    ...
    public ParticleExplosionThermobaric(double x, double y, double z, double explosionSize) {
        buf().writeDouble(x);
        buf().writeDouble(y);
        buf().writeDouble(z);
    ...
    public void client(EntityPlayer player) {
        ...
        if (Minecraft.getMinecraft().gameSettings.fancyGraphics) {
            if (!player.worldObj.getBlock((int) x, (int) y, (int) z).getMaterial().isLiquid()) {
                for (int i = 0; i < 18 * explosionSize; i++) {
                    Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleFlame(player.worldObj, x + ((i * 0.2f) * (Main.rand.nextGaussian() - Main.rand.nextGaussian())) * 0.1f, y + ((i * 0.2f) * (Main.rand.nextGaussian() - Main.rand.nextGaussian())) * 0.1f + 0.5f, z + ((i * 0.2f) * (Main.rand.nextGaussian() - Main.rand.nextGaussian())) * 0.1f, (Main.rand.nextGaussian() - Main.rand.nextGaussian()) * 0.1f, (Main.rand.nextGaussian() - Main.rand.nextGaussian()) * 0.1f, (Main.rand.nextGaussian() - Main.rand.nextGaussian()) * 0.1f, Math.max((float) explosionSize * (1.6f + i * 0.005f), 10.0f)));
                }
            }
        }
    }
