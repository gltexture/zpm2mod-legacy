...
public class EntityBastionRocket extends EntityDetectableRocketZp {
    ...
    public EntityBastionRocket(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityBastionRocket(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public void explode() {
        ExplosionUtils.makeExplosion(this.worldObj, null, this, this.posX, this.posY + (double) (this.height / 16.0F), this.posZ, 10.0f, false, true, false);
        this.setDead();
    }
    ...
    public void spawnParticles() {
        ...
        float flameScale = (4.0f * multiplier) + 0.25f;
        for (int i = 0; i < 2; i++) {
            Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleColoredCloud(this.worldObj, this.posX - this.motionX * 2.0f, this.posY - this.motionY * 2.0f, this.posZ - this.motionZ * 2.0f, -this.motionX * 0.2f, -this.motionY * 0.2f, -this.motionZ * 0.2f, new float[]{0.85f, 0.85f, 0.85f}, 1.2f));
        }
                ...
                if (this.seekPunishment == 0) {
                    if (!this.worldObj.isAirBlock(this.defenceToDestroy.xCoord, this.defenceToDestroy.yCoord, this.defenceToDestroy.zCoord)) {
                        if (this.getDistance(this.defenceToDestroy.xCoord, this.defenceToDestroy.yCoord, this.defenceToDestroy.zCoord) > 5) {
                            ...
                            this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, 3.8f, 0.0f);
                        } else {
                            this.punishSeek();
                        }
                    }
        ...
        if (!list.isEmpty()) {
            for (TileEntity tileEntity : list) {
                if (tileEntity instanceof TileTacticalBlock) {
                    ...
                    if (this.getDistance(tileAD.xCoord, this.posY, tileAD.zCoord) <= 54) {
