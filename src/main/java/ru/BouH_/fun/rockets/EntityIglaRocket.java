...
public class EntityIglaRocket extends EntityDefenceRocketZp {
    public static final Set<Class<? extends EntityDetectableRocketZp>> rockets = new HashSet<>();
    ...
    public EntityIglaRocket(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityIglaRocket(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public EntityIglaRocket(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_) {
        super(p_i1756_1_, p_i1756_2_, 3.9f, p_i1756_3_);
    }
    ...
    public void onUpdate() {
        ...
        if (this.entityToDestroy != null && this.entityToDestroy.isEntityAlive()) {
            if (this.seekPunishment == 0) {
                if (!this.entityToDestroy.isDead && this.isCharging) {
                    if (this.getDistanceToEntity(this.entityToDestroy) <= 2.0f) {
                        if (this.entityToDestroy instanceof EntityRocketZPBase) {
                            ((EntityRocketZPBase) this.entityToDestroy).onImpact(null);
                        }
                        this.onImpact(null);
                    }
                    ...
                    double atY = this.entityToDestroy.posY + this.entityToDestroy.getEyeHeight();
    ...
    public void setInfo(Entity entityHit) {
        this.entityToDestroy = entityHit;
        if (entityHit instanceof EntityDetectableRocketZp) {
            EntityDetectableRocketZp rocketDetectable = (EntityDetectableRocketZp) entityHit;
            if (rocketDetectable.canBeDetected() && !rocketDetectable.isDetected()) {
