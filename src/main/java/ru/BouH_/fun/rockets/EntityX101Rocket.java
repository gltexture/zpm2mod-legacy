...
public class EntityX101Rocket extends EntityDetectableRocketZp {
    public EntityLivingBase entityToDestroy;
    private int seekPunishment;

    public EntityX101Rocket(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityX101Rocket(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
            ...
            if (this.entityToDestroy != null && !this.entityToDestroy.isDead) {
                if (this.seekPunishment == 0) {
                    if (this.getDistanceToEntity(this.entityToDestroy) > 3) {
                        ...
                        if (this.getDistance(atX, plY, atZ) <= 8) {
                            this.motionY = -MathHelper.sin((this.rotationPitch + this.func_70183_g()) / 180.0F * (float) Math.PI);
                        } else {
                            this.motionY *= 0.1f;
                        }

                        if (this.homingRocket != null) {
                            if (this.motionX != 0 && this.motionZ != 0) {
                                if (this.getDistanceToEntity(this.homingRocket) <= 5.0f) {
                                    this.motionY = 1.5f;
                                    this.punishSeek();
    ...
    private EntityLivingBase seekForEnemy() {
        List<EntityPlayer> list = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.boundingBox.expand(64, 256, 64));
        if (!list.isEmpty()) {
