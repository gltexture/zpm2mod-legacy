...
public class EntityGlassBottle extends EntityThrowableZPBase {
    public EntityGlassBottle(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityGlassBottle(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public EntityGlassBottle(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_) {
        this(p_i1756_1_, p_i1756_2_, p_i1756_3_, 10.0f);
    }

    public EntityGlassBottle(World p_i1756_1_, EntityLivingBase p_i1756_2_, float speed, float inac) {
    ...
    public void spawnParticlesImpact(double x, double y, double z) {
        for (int i = 0; i < 16; ++i) {
            Minecraft.getMinecraft().effectRenderer.addEffect(new EntityBreakingFX(this.worldObj, x, y, z, (Main.rand.nextGaussian() - this.motionX) * 0.05D, (Main.rand.nextGaussian() - this.motionY) * 0.05D + 0.1f, (Main.rand.nextGaussian() - this.motionZ) * 0.05D, Items.glass_bottle, 1));
        }
    }
                ...
                if (!(this.getThrower() instanceof AZombieBase && entity instanceof AZombieBase)) {
                    if (!this.worldObj.isRemote) {
                        if (this.getThrower() instanceof EntityPlayer) {
                            DamageSource damagesource = DamageSourceZp.causePlayerGlassBottleDamage((EntityPlayer) this.getThrower());
