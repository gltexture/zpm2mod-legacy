...
public class EntityCustomRocket extends EntityRocketZPBase {

    public EntityCustomRocket(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityCustomRocket(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public EntityCustomRocket(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_) {
        super(p_i1756_1_, p_i1756_2_, 1.8f, p_i1756_3_);
    }

    @SideOnly(Side.CLIENT)
    public void spawnParticles() {
        float multiplier = Math.min((float) ((this.chargingTicks() + 15) - this.ticksExisted) / this.chargingTicks(), 1.0f);
        int flameCount = (int) (12 * multiplier) + 1;
        float flameScale = (2.0f * multiplier) + 0.25f;
        for (int i = 0; i < 4; i++) {
    ...
    public void explode() {
        ...
        ExplosionUtils.makeExplosion(this.worldObj, player, this, this.posX, this.posY + (double) (this.height / 16.0F), this.posZ, 4.0f, false, true, false);
        this.setDead();
    }
