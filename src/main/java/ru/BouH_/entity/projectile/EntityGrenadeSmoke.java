...
public class EntityGrenadeSmoke extends EntityThrowableZPBase {
    private boolean started;

    public EntityGrenadeSmoke(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityGrenadeSmoke(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public EntityGrenadeSmoke(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_) {
        this(p_i1756_1_, p_i1756_2_, p_i1756_3_, 10.0f);
    }

    public EntityGrenadeSmoke(World p_i1756_1_, EntityLivingBase p_i1756_2_, float speed, float inac) {
    ...
    public void spawnParticles() {
        for (int i = 0; i < 2; i++) {
            Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleColoredCloud(this.worldObj, this.posX + rand.nextFloat() * 6 - 3.0f, this.posY + 0.75f * i, this.posZ + rand.nextFloat() * 6 - 3.0f, rand.nextFloat() * 0.3f - 0.15f, 0.15f, rand.nextFloat() * 0.3f - 0.15f, new float[]{0.85f, 0.85f, 0.85f}, 3.5f));
        }
    }

    public void onUpdate() {
        super.onUpdate();
        if (this.isBurning() || this.ticksExisted >= 500) {
            this.setDead();
        }
        ...
        if (this.isStarted()) {
            if (this.worldObj.isRemote) {
                this.spawnParticles();
                this.playSound(Main.MODID + ":grenade_detonate", 1.5F, 0.8F / (this.rand.nextFloat() * 0.2F + 0.4F));
            }
        }
    }
