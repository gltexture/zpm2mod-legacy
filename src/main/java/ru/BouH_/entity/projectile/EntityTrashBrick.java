...
public class EntityTrashBrick extends EntityThrowableZPBase {
    public EntityTrashBrick(World p_i1776_1_) {
        super(p_i1776_1_);
    }

    public EntityTrashBrick(World p_i1778_1_, double p_i1778_2_, double p_i1778_4_, double p_i1778_6_) {
        super(p_i1778_1_, p_i1778_2_, p_i1778_4_, p_i1778_6_);
    }

    public EntityTrashBrick(World p_i1756_1_, EntityLivingBase p_i1756_2_, float p_i1756_3_) {
        this(p_i1756_1_, p_i1756_2_, p_i1756_3_, 10.0f);
    }

    public EntityTrashBrick(World p_i1756_1_, EntityLivingBase p_i1756_2_, float speed, float inac) {
    ...
    public void spawnParticlesImpact(double x, double y, double z) {
        for (int i = 0; i < 8; ++i) {
            Minecraft.getMinecraft().effectRenderer.addEffect(new EntityBreakingFX(this.worldObj, x, y, z, (Main.rand.nextGaussian() - this.motionX) * 0.05D, (Main.rand.nextGaussian() - this.motionY) * 0.05D + 0.1f, (Main.rand.nextGaussian() - this.motionZ) * 0.05D, Items.brick, 1));
        }
    }
    ...
    protected void onImpact(MovingObjectPosition movingobjectposition) {
        if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) {
            ...
            if (!this.worldObj.isRemote) {
                this.worldObj.playSoundAtEntity(this, "dig.stone", 1F, 0.8F);
                if (this.worldObj.getGameRules().getGameRuleBooleanValue("zombiesCanMine") && this.getThrower() instanceof AZombieBase && hardness >= 0 && hardness <= 50 && rand.nextInt(hardness + 4) == 0) {
