...
public class ItemOldBackpack extends Item {
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            for (int i = 0; i < Main.rand.nextInt(3) + 2; i++) {
                float f1 = Main.rand.nextFloat();
                List<LootSpawnManager> list = new ArrayList<>(f1 <= 0.1f ? EnumLootGroups.Tier1Police.getLSP() : f1 <= 0.4f ? EnumLootGroups.Tier2Village.getLSP() : f1 <= 0.85f ? EnumLootGroups.Tier1City.getLSP() : EnumLootGroups.Tier1MegaCity.getLSP());
