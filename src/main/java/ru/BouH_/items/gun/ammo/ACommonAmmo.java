...
public class ACommonAmmo extends AAmmo {
    private final float volume;
    private final Shell shell;

    public ACommonAmmo(Item item, Shell shell, float volume) {
    ...
    public void generateSmoke(EntityPlayer player, ItemStack stack, AGunBase aGunBase, int count) {
        ...
        for (int i = 0; i < count; i++) {
            if (player.isInsideOfMaterial(Material.water)) {
                ...
                Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleGunSmoke(player.worldObj, posX, posY, posZ, lookVec.xCoord * 0.5f, lookVec.yCoord * 0.5f, lookVec.zCoord * 0.5f));
            }
        }
    }
