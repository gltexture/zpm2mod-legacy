...
public class ACustomAmmo extends AAmmo {
    private final float volume;
    private final Shell shell;

    public ACustomAmmo(Item item, Shell shell, float volume) {
    ...
    public void generateSmoke(EntityPlayer player, ItemStack stack, AGunBase aGunBase, int count) {
        ...
        for (int i = 0; i < count + 4; i++) {
            if (player.isInsideOfMaterial(Material.water)) {
                ...
                float f = 0.3f + i * 0.5f;
                Minecraft.getMinecraft().effectRenderer.addEffect(new EntityParticleCustomAmmoCloud(player.worldObj, posX, posY, posZ, lookVec.xCoord * 0.1f * f, lookVec.yCoord * 0.1f * f, lookVec.zCoord * 0.1f * f, new float[]{0.9f, 0.9f, 0.9f}));
            }
        }
    }
