...
public class ItemSniperRifleNomag extends ItemSniperRifle {
    protected String reloadSound2;

    public ItemSniperRifleNomag(String name, String shootSound, String reloadSound, int ammo, int cdReload, int damage, int cdShoot, int maxDistance, int effectiveDistance, float recoilVertical, float recoilHorizontal, float inaccuracy, float inaccuracyInAim, float addInaccuracyStep, float maxAddInaccuracy, float jamChance, boolean shouldDropShell, boolean isAutomatic, AAmmo ammoItem) {
        ...
        this.setAdditionalReloadSound("shell_r");
    }
    ...
    public List<String> weaponDescriptionText(ItemStack stack) {
        ...
        list.add(String.format("%s: %s %s", I18n.format("weapon.gui.desc.damage"), RenderUtils.getModifiedValueBiggerWorse(this.getDamage(), this.collectModuleDamage(stack)), I18n.format("weapon.gui.desc.hp")));
        list.add(String.format("%s: %s %s", I18n.format("weapon.gui.desc.damagePerSec"), RenderUtils.getModifiedValueBiggerWorse(20.0f * this.getDamage() / this.getFireRate(), this.collectModuleDamage(stack)), I18n.format("weapon.gui.desc.hp")));
    ...
    public void onReloadPost(ItemStack stack, EntityPlayer player) {
        SoundUtils.playSound(player, (Main.MODID + ":" + "sniper_r"), 2.5f, this.getScaledPitch(player));
    }
}
