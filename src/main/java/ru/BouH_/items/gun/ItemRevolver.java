...
public class ItemRevolver extends ItemPistol {
    protected String reloadSound2;

    public ItemRevolver(String name, String shootSound, String reloadSound, int ammo, int cdReload, int damage, int cdShoot, int maxDistance, int effectiveDistance, float recoilVertical, float recoilHorizontal, float inaccuracy, float inaccuracyInAim, float addInaccuracyStep, float maxAddInaccuracy, float jamChance, boolean shouldDropShell, boolean isAutomatic, AAmmo ammoItem) {
    ...
    public void onReloadPost(ItemStack stack, EntityPlayer player) {
        SoundUtils.playSound(player, (Main.MODID + ":" + "reload_r"), 2.5f, this.getScaledPitch(player));
    }
}
